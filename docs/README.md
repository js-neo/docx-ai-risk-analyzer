...
Текущий Git commit нужно проверять через `git log --oneline -n 5`, а remote baseline — после `git push`. Не нужно обновлять этот index только ради смены commit hash, если не меняется документационная карта или active workflow.
...
| `docs/workflow/project-development-rules.md` | Общие правила разработки: DDD-inspired architecture, boundaries, safety, non-regression and checks | При изменении архитектурных границ, DDD rules, module conventions, privacy/safety rules или required checks |
| `docs/workflow/chatgpt-implementation-output-protocol.md` | Формат текущей разработки через ChatGPT до подключения Codex | При изменении формата вывода коммитов, модулей, проверок, shell commands или chat-based implementation workflow |
| `docs/workflow/github-pr-orchestration.md` | Правила GitHub PR workflow, review ownership, CI, comments and merge decision boundaries | При изменении PR workflow, review decision levels, CI policy, branch/PR rules или merge decision boundaries |
| `docs/workflow/pr-body-and-extended-description-contract.md` | Контракт English-first GitHub-facing PR body и Extended description при owner-managed workflow | При изменении PR body ownership, Extended description, Codex report boundaries, GitHub-facing language rules или merge/review comment preparation |
...
релевантный PR diff, CI logs и review comments
~~~

### PR body or Extended description task
...
релевантный PR diff, CI logs, Codex report и review comments
~~~
...
До подключения Codex ChatGPT должен работать по документу:
...
Codex не должен подключаться к полноценной реализации до появления и approval полного набора workflow-документов:
...
`docs/workflow/github-pr-orchestration.md` описывает PR workflow. `docs/workflow/pr-body-and-extended-description-contract.md` фиксирует owner-managed подготовку English-first GitHub-facing PR body и Extended description. Codex-specific implementation rules and prompt templates должны оставаться отдельными документами.
